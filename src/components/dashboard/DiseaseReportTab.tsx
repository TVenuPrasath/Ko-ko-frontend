      ...
      <div className="bg-white rounded-2xl border border-border/60 shadow-sm overflow-hidden">
        ...
        <div className="p-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="bg-muted/40 rounded-xl px-3 py-2.5 text-sm font-medium text-foreground border border-border/40">
              {user.hamlet}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-1">
              {SYMPTOM_KEYS.map((key) => (
                <label
                  key={key}
                  className={`flex items-center justify-between py-2.5 px-3 rounded-xl cursor-pointer transition-colors border ${
                    symptoms[key] ? "bg-danger/5 border-danger/30" : "border-transparent hover:bg-muted/30"
                  }`}
                >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input
                ...
                inputMode="numeric"
                className="text-center text-base font-bold border-2 focus:border-danger rounded-xl"
                placeholder="0"
              />
            </div>
            <div>
          ...
          {/* Notes */}
          <div>
            <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">கூடுதல் குறிப்பு / Other notes</label>
            <Textarea
              value={otherNote}
              onChange={(e) => setOtherNote(e.target.value)}
              placeholder="எ.கா. கோழிகள் தீவனம் சாப்பிடவில்லை, வேறு நோய் அறிகுறிகள்..."
              rows={3}
              className="text-sm resize-none border-2 focus:border-danger rounded-xl"
            />
          </div>
          ...
          <Button
            ...
            disabled={loading}
            className="tap-target w-full text-base font-bold rounded-xl shadow-sm bg-danger text-danger-foreground hover:bg-danger/90 disabled:opacity-40"
          >
